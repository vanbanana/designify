...
## 卡片高度规则（等高 vs 不等高 — 最容易滥用的设计决策）

| 场景 | 卡片高度 | 理由 |
|------|---------|------|
| 功能特性/服务介绍（3-6 张卡片） | **必须等高** | 用户横向对比功能，不等高导致视觉混乱 |
| 定价方案（3 档对比） | **必须等高** | 核心对比场景，不等高暗示"这个更差/更好" |
| 团队成员介绍 | **必须等高** | 列表性展示，不等高破坏一致性 |
| 作品集/画廊（摄影/设计/艺术） | **建议不等高** | 不同作品有不同比例，不等高体现内容自然差异 |
| 瀑布流/Pinterest 式 | **强制不等高** | 天然瀑布流，等高反而失真 |
| 博客/文章卡片 | **可等高（推荐）** | 标题长度差异用 ellipsis 或 min-height 控制 |

```
铁律: 功能类卡片（特性/定价/团队）必须等高对齐。不等高 = 布局错误。
铁律: 作品集/画廊场景可用不等高，但必须基于内容自然差异，非故意随机。
铁律: 同一行内所有卡片宽度必须一致。仅瀑布流场景允许不等宽。
```
